...
| Profile | 参数 |
|---|---|
| `web` | `--host`、`--port`、可重复的 `--trusted-host`、`--trust-privileged-methods` |
...
生产 Web 运行器需要已构建的包和前端产物（`pnpm run build`）。默认服务地址是 `http://127.0.0.1:3080`。CLI 目前有意不支持 `--host 0.0.0.0`，并会以用法错误退出；`--trusted-host` 可添加 `/api` 浏览器信任围栏接受的具名 authority。这些 authority 仍然够不到 settings 与 credential 方法——它们依旧钉死在回环地址上：`--trust-privileged-methods` 才会解除这道钉死，而且只对已声明的 authority 生效。它是一项断言：能连到这个端口本身就已经过另一层认证（带 ACL 的 VPN 接口、做认证的反向代理）——绝不是用来打开一个普通局域网或公网端口的开关；不搭配任何 `--trusted-host` 时它是用法错误，因为单独出现时它什么都不授予。
