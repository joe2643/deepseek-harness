...
- **Video promotion needs a deployment-supplied resolver** — `PiAiAdapterOptions.resolveVideoUrl` is the only way a video reaches a provider as a `video_url` address instead of an inline base64 data URI, and nothing in this package signs or serves one. Without it every video inlines, which costs a request body 4/3 the file size; one measured gateway accepted an 8.7 MB body and returned HTTP 413 for a 13.0 MB one, so a long clip needs either a resolver or frame sampling. A resolver returning a loopback origin is refused and inlined, because a remote provider cannot fetch it.
