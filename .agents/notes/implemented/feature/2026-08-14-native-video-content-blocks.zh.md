...
**视频可以用 URL 而非字节传输。** 内联自足且随时可用，但请求体是文件的 4/3，而网关会拒绝过大的请求体：某部署实测，8.7 MB 的请求体成功，13.0 MB 则返回 HTTP 413 `RequestTooLarge`。因此 `PiAiAdapterOptions.resolveVideoUrl` 让部署方把持久化视频提升为提供方自行抓取的地址；同一段内联失败的 13 MB 片子以 URL 方式成功，而一段 963 KB 的片子只用了 390 字节的请求。提升在两个方向上都是可选的 —— 本包既不签名也不托管，而 resolver 若拒绝、抛错或返回 loopback 源，都会回退到内联，因为提供方取不到的 URL 绝不该顶替本来可用的字节。
